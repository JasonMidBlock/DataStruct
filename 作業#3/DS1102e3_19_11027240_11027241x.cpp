...
int challenge_calculate(Node* Prefix, bool &error);
void genIO();
...
void Node_push_front(Node* &head, string s){
	Node* temp = new Node;
	temp->str = s;
	temp->next = head;
	head = temp;
...
void genIO(){
	...
	cout << "* 2. Infix2prefix Evaluation      *\n";
	cout << "***********************************\n\n";
}
...
void challenge_calculate(Node* head){
	Stack sta;
	int num1, num2;
	while(head != NULL){
		if( head-> str == "+"){
...
void challenge(string str){
		...
		if(char_is_operation(s[i][0]) || char_is_brackets(s[i][0])){
			if(sta.s_empty())
				...
				if(sta.s_top() == ")" || sta.s_top() == "+" || sta.s_top() == "-")
					...
					while( !sta.s_empty() && sta.s_top() != ")" && sta.s_top() != "+" && sta.s_top() != "-" ){
						prefix.s_push(sta.s_top()); // Node_push(Prefix, sta.s_top());
						sta.s_pop();
					}
				...
				if(sta.s_top() == ")" || sta.s_top() == "+" || sta.s_top()  == "-" || sta.s_top() == "*" || sta.s_top()  == "/")
					sta.s_push(s[i]);
				else{
					while(!sta.s_empty() && (sta.s_top() == "*" || sta.s_top() == "/") ){	
						prefix.s_push(sta.s_top()); // Node_push(Postfix, sta.s_top());
						sta.s_pop();
					}
			...
			prefix.s_push(s[i]); // Node_push(Postfix, s[i]);
		}
	...
	while(!sta.s_empty()){
		prefix.s_push(sta.s_top());
		sta.s_pop();
	}
	
	
	while( !prefix.s_empty() ){
		Node_push(Prefix, prefix.s_top());
		prefix.s_pop();
	}
	...
	Node* n = NULL;
	while(Prefix != NULL){
		Node_push_front(n, Prefix-> str);
		Prefix = Prefix-> next;
	}
	
	challenge_calculate(n);
}
