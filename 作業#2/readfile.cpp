...
Readfile mergefile(Readfile file1, Readfile file2){  // *
	...
	while(num1 < file1.datas.size() || num2 < file2.datas.size() ){
		if(file2.datas.empty()){
			file3.datas.push_back(file1.datas[num1]);
			num1 ++;
		}
		else if(file1.datas.empty()){
			file3.datas.push_back(file2.datas[num2]);
			file2.datas.erase(file2.datas.begin() + num2);
		}
		else if( atoi(file1.datas[num1].school_code.c_str()) < atoi(file2.datas[num2].school_code.c_str()) ){
			file3.datas.push_back(file1.datas[num1]);
			num1 ++;
		}
		else if(atoi(file1.datas[num1].school_code.c_str()) == atoi(file2.datas[num2].school_code.c_str())){
			if(atoi(file1.datas[num1].major_code.c_str()) < atoi(file2.datas[num2].major_code.c_str())){
				file3.datas.push_back(file1.datas[num1]);
				num1 ++;
			}
			
			else if(atoi(file1.datas[num1].major_code.c_str()) == atoi(file2.datas[num2].major_code.c_str())){
				file3.datas.push_back(file1.datas[num1]);
				num1 ++;
				file3.datas.push_back(file2.datas[num2]);
				file2.datas.erase(file2.datas.begin() + num2);
...
void mission4(){
	...
	ifstream ifs;
	ofstream ofs;
	int times = Input_A_positive_Num();
	
	for(int i =0 ;i< times -1; i++){
		while(true){
			cout << "Input file - 201, 202, ...[0]Quit):";
			cin >> s1;
			if(atoi(s1.c_str()) == 0 )
				return ;
			ifs.open(s1.c_str());
	...
	while(true){
		cout << "Input 3rd file - 201, 202, ...[0]Quit):";
		cin >> s3;
		if (atoi(s3.c_str() ) == 0)
			return;
		ifs.open(s3.c_str());
	...
	Readfile file4 = mergefile(file1, file2);
	file4 = mergefile(file4, file3);
	cout << "\noutput\n";
	cout << "Total number of records = "<< file3.datas.size() << "\n";
	
	string file4_name = "output";
	for(int i =0 ; i< file1.file_name.length(); i++){
		if(isdigit(file1.file_name[i]))
			file4_name += file1.file_name[i];
	}
	file4_name += '_';
	
	for(int i =0 ; i< file2.file_name.length(); i++){
		if(isdigit(file2.file_name[i]))
			file4_name += file2.file_name[i];
	}
	file4_name += '_';
	
	for(int i =0 ; i< file3.file_name.length(); i++){
		if(isdigit(file3.file_name[i]))
			file4_name += file3.file_name[i];
	}
