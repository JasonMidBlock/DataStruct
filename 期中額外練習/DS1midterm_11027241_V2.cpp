...
class Primes : public Fib{
	private:
		BIG_INT prime_number[MAX_NO];
	public:
		bool IsPrime(int i){
			if(i <= 1)
				return false;
		...
		void Prime_Put(int num, int postition){
			prime_number[postition] = num;
		}
		
		Primes() {									// constructor to initialize data
			cout << "All prime numbers:\n";
			int total =0;
			for ( int i = 1; i < MAX_NO; i++ ) {
	    		if ( IsPrime( get( i ) ) ) {
	    			Prime_Put( total, get( i ) );
	    			cout << "[" << total << "] " << get( i ) << endl;
	    			total++;
				} // if
